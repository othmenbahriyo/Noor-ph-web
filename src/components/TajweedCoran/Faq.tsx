...
export default function Faq() {
  const t = useTranslations('tajweedCoran.faq');
  ...
  const toggle = (index: number) => {
    setOpenIndex((current) => {
      ...
      if (next !== null) trackEvent('faq_question_open', { question: items[index].question, page: 'tajweed_coran' });
      return next;
    });
  };
    ...
    <div className={`container ${contentStyles.containerWrap}`}>
      <div className={`${contentStyles.contentBlock} ${styles.faqBlock}`}>
        ...
        {items.map((item, index) => {
          ...
          const answerId = `tajweed-faq-answer-${index}`;
